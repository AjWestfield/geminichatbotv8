...
-- First, check for orphaned images
SELECT COUNT(*) as orphaned_images_count
FROM images
WHERE chat_id IS NULL;

-- Create a default chat for orphaned images if needed
DO $$
DECLARE
...
-- Create or replace function to get all images
...
-- Update the chat_summaries view
...
-- Add index to improve image loading performance
CREATE INDEX IF NOT EXISTS idx_images_chat_id_created_at 
ON images(chat_id, created_at DESC);

-- Function to ensure image has valid chat association
...
-- Create trigger to ensure images always have a chat
...
-- Function to fix orphaned images
...
-- Ensure all existing images have proper associations
...
-- Show recent images for verification
SELECT 
    'Recent Images' as section,
    id,
    chat_id,
